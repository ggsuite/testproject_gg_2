...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/testproject_gg_2.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final testprojectGg2 = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'testproject_gg_2.dart',
      );

      expect(
        testprojectGg2,
        isNotNull,
        reason: 'Item for executing bin/testproject_gg_2.dart not found',
      );

      expect(testprojectGg2['name'], 'testproject_gg_2.dart');
      expect(testprojectGg2['type'], 'dart');
      expect(testprojectGg2['request'], 'launch');
